...
std::optional<Row> MergeTreePartition::tryParseValueFromID(const String & partition_id, const Block & partition_key_sample)
{
    ...
    /// Try to parse partition key value from partition_id if all columns are numeric.
    for (size_t i = 0; i < num_keys; ++i)
    {
        ...
        if (sample_field.getType() != Field::Types::UInt64 && sample_field.getType() != Field::Types::Int64)
            return {};
